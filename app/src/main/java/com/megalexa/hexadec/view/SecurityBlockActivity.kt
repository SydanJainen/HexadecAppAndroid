...
class SecurityBlockActivity : AppCompatActivity(),MainContract.BlockView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.securityview)
        setSupportActionBar(my_toolbar)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.title = getString(R.string.app_name)
        setPresenter(BlockPresenter(this@SecurityBlockActivity))
        sBlockbtnSalvab.setOnClickListener {
            val intent=Intent(this@SecurityBlockActivity, AddBlocksActivity::class.java)
            intent.putExtra("Workflow",newWorkflow)
            startActivity(intent)
        }
        sBlockbtnSalvaw.setOnClickListener {
            newWorkflow=presenter.addBlock(newWorkflow,findViewById<EditText>(R.id.textBlockConf).text.toString(),"SECURITY")
            presenter.saveWorkflow(newWorkflow)
            startActivity(Intent(this@SecurityBlockActivity, ViewActivity::class.java))
        }
        sBlockbtnAnnulla.setOnClickListener {
            val intent=Intent(this@SecurityBlockActivity, NewWorkflowActivity::class.java)
            intent.putExtra("Workflow",newWorkflow)
            startActivity(intent)
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if ( item.itemId == R.id.settings) {
            startActivity(Intent(this@SecurityBlockActivity, SecurityBlockActivity::class.java))
            return true
        }
    ...
    private fun startToast(message:String) {
        runOnUiThread {
            Toast.makeText(this@SecurityBlockActivity,message, Toast.LENGTH_LONG).show()
        }
    }
}