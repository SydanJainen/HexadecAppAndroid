...
class SlackBlockActivity : AppCompatActivity(),MainContract.BlockView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.slackview)
        setSupportActionBar(my_toolbar)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.title = getString(R.string.app_name)
        setPresenter(BlockPresenter(this@SlackBlockActivity))
        sBlockbtnSalvab.setOnClickListener {
            val intent=Intent(this@SlackBlockActivity, AddBlocksActivity::class.java)
            intent.putExtra("Workflow",newWorkflow)
            startActivity(intent)
        }
        sBlockbtnSalvaw.setOnClickListener {
            newWorkflow=presenter.addBlock(newWorkflow,findViewById<EditText>(R.id.textBlockConf).text.toString(),"SLACK")
            //presenter.saveWorkflow(newWorkflow)
            startActivity(Intent(this@SlackBlockActivity, ViewActivity::class.java))
        }
        sBlockbtnAnnulla.setOnClickListener {
            val intent=Intent(this@SlackBlockActivity, NewWorkflowActivity::class.java)
            intent.putExtra("Workflow",newWorkflow)
            startActivity(intent)
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if ( item.itemId == R.id.settings) {
            startActivity(Intent(this@SlackBlockActivity, SlackBlockActivity::class.java))
            return true
        }
    ...
    private fun startToast(message:String) {
        runOnUiThread {
            Toast.makeText(this@SlackBlockActivity,message, Toast.LENGTH_LONG).show()
        }
    }
}